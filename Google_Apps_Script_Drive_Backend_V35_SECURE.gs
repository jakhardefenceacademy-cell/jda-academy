...
function doPost(e){try{const body=JSON.parse((e&&e.postData&&e.postData.contents)||'{}');if(!auth_(body.token))return json_({ok:false,error:'Unauthorized'});if(body.action==='fetchVacancies'){
  ...
  if(code<200||code>=300) return json_({ok:false,error:'Feed HTTP '+code});
  return json_({ok:true,items:parseFeed_(resp.getContentText())});
}
if(body.action==='initDrive'){const f=folder_();return json_({ok:true,folderName:FOLDER,folderId:f.getId(),googleAccount:body.googleAccount||''});}if(body.data){save_(body.data);return json_({ok:true,data:load_(),savedAt:new Date().toISOString()});}return json_({ok:true,data:load_()});}catch(err){return json_({ok:false,error:String(err)});}}
...
function parseFeed_(xml){
  const out=[], re=/<item\b[\s\S]*?<\/item>/gi; let m;
  while((m=re.exec(xml))!==null && out.length<100){
    const item=m[0];
    const val=(tag)=>{const r=new RegExp('<'+tag+'[^>]*>([\\s\\S]*?)<\\/'+tag+'>','i').exec(item);return r?r[1].replace(/<!\[CDATA\[([\s\S]*?)\]\]>/g,'$1').trim():''};
    const title=val('title'), link=val('link'), description=val('description'), date=val('pubDate')||val('published')||new Date().toISOString();
    if(title) out.push({title:title.replace(/<[^>]*>/g,''),link,details:description.replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').trim(),date});
  }
  return out;
}
